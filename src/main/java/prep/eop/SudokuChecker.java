...
// Check region constraints.
    ...
    public static void main(String[] args) {
        // traverse in CNN fashion
        int size = 9;
        int regionSize = 3;//(int) Math.sqrt(size);  //+ regionSize <= size  < size - 1
        for (int I = 0; I + regionSize <= size ; ++I) {
            for (int J = 0; J + regionSize <= size ; ++J) {
                System.out.println(""+I+" "+(I+regionSize)+" "+J+" "+(J+regionSize));
            }
        }
    }
}
